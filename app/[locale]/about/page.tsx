...
const EXPERIENCE: {
...
}[] = [
  {
    ...
    bullets: [
      ...
      'LLM Translation System — RAG 및 Graph 리소스를 포함한 HITL LLM 번역 플랫폼 구축, 연간 1.2억 절감 (2025년 기준 4개 게임 적용)',
      'Domain Specific Benchmark — HITL 파이프라인이 포함된 게임 도메인 번역 벤치마크 시스템 설계 및 개발',
    ],
    bulletsEn: [
      'MapleStory: Idle RPG — Managed end-to-end localization including text data management, budget control, schedule tracking, and multilingual vendor coordination',
      'LLM Translation System — Built HITL LLM translation platform with RAG & Graph resources, reducing annual costs by ¥120M (applied to 4 games as of 2025)',
      'Domain Specific Benchmark — Designed and developed a game domain translation benchmark system with HITL pipeline',
    ],
...
const EDUCATION: {
  ...
  { degree: 'B.A.', school: 'Hanyang University ERICA', dept: 'French Studies', period: 'Mar 2017 — Feb 2024' },
  { degree: 'B.S. (Dual)', school: 'Hanyang University ERICA', dept: 'Software Engineering', period: 'Mar 2017 — Feb 2024' },
]
...
const CERTIFICATIONS: {
  ...
  { name: 'Hugging Face Agents Course', issuer: 'Hugging Face', date: 'Feb 2026' },
  { name: 'AWS Certified AI Practitioner', issuer: 'Amazon Web Services', date: 'Jan 2026', expires: 'Jan 2029' },
  { name: 'Azure AI Fundamentals', issuer: 'Microsoft', date: 'Dec 2024' },
  { name: 'Google Analytics Certification', issuer: 'Google', date: 'Feb 2025', expires: 'Feb 2026' },
]
...
const ACTIVITIES: {
...
}[] = [
  {
    ...
    descEn: 'Research on building a Korean cultural content translation system that goes beyond standard LLMs.',
  },
  {
    ...
    descEn: 'Developing an AI-powered product to bridge the information gap for socially disadvantaged groups.',
  },
  {
    title: 'ABB Hackathon',
    period: '2023',
  ...
  },
  {
    ...
    descEn: 'Peer tutoring for international students.',
  },
]
...
const ORGANIZATIONS: {
...
}[] = [
  {
    ...
    desc: '대학 연합 빅데이터 동아리',
    descEn: 'Inter-university Big Data club',
  },
  {
    name: 'Typhoon',
    role: 'Tech Leader',
    ...
    desc: '중앙 동아리 · 농구',
    descEn: 'Central University Club · Basketball',
  },
]

const SKILLS = [
  { label: 'Localization', items: ['L10N', 'I18N', 'TMS', 'Game Localization', 'CAT Tools', 'Vendor Management'] },
  { label: 'AI / ML', items: ['LLM', 'RAG', 'Fine-tuning', 'Prompt Engineering', 'HuggingFace', 'Azure AI'] },
  { label: 'Data & Backend', items: ['Python', 'FastAPI', 'PostgreSQL', 'Pandas', 'PyTorch', 'SQL'] },
  { label: 'Infra', items: ['AWS', 'Docker', 'GitHub Actions', 'Snowflake', 'Linux', 'Git'] },
]
    ...
    <div className="max-w-3xl mx-auto px-6 py-10">
      ...
      <section className="flex flex-col sm:flex-row items-start gap-6 mb-10 pb-10 border-b border-slate-800/60">
        <div className="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 relative ring-1 ring-slate-700/50">
          <Image src="/blog-profile.png" alt="Jaejuna" fill className="object-cover" priority />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
              {[
                { label: '[github]', href: 'https://github.com/Jaejuna' },
                { label: '[linkedin]', href: 'https://www.linkedin.com/in/jaejun-jung-37107a293/' },
                { label: '[email]', href: 'mailto:anayana9988@gmail.com' },
              ].map(({ label, href }) => (
          ...
          <p className="text-sm text-slate-400 mt-4 leading-relaxed">{t('bio')}</p>
          <div className="flex flex-wrap gap-4 mt-4">
            <span className="font-mono text-xs text-slate-500"><span className="text-accent-700">❯</span> Pangyo, South Korea</span>
            <span className="font-mono text-xs text-slate-500"><span className="text-accent-700">❯</span> {t('openToConnect')}</span>
          </div>
        </div>
      </section>
      ...
      <section className="mb-10 pb-10 border-b border-slate-800/60">
        ...
        <div>
          {EXPERIENCE.map((exp, i) => {
            const bullets = isEn ? (exp.bulletsEn ?? exp.bullets) : exp.bullets
            const desc = isEn ? (exp.descEn ?? exp.desc) : exp.desc
            return (
              <TimelineItem key={exp.company + exp.role} isLast={i === EXPERIENCE.length - 1}>
                <div className="flex items-start justify-between flex-wrap gap-1 mb-1">
                  <div>
                    ...
                    <p className="font-mono text-xs text-accent-500 mt-0.5">
                      {exp.company}
                      {exp.type && <span className="text-slate-700 ml-2">· {exp.type}</span>}
                    </p>
                  </div>
                  <span className="font-mono text-xs text-slate-600">{exp.period}</span>
                </div>
                {desc && <p className="text-sm text-slate-400 leading-relaxed mt-1 mb-2">{desc}</p>}
                ...
                {exp.tags && exp.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {exp.tags.map((t) => (
                      <span key={t} className="font-mono text-xs text-slate-500 bg-slate-800/40 border border-slate-800/60 px-2 py-0.5 rounded">{t}</span>
                    ))}
                  </div>
                )}
              </TimelineItem>
            ...
            <TimelineItem key={edu.degree + edu.dept} isLast={i === EDUCATION.length - 1}>
              <div className="flex items-start justify-between flex-wrap gap-1">
                <div>
                  ...
                  <p className="font-mono text-xs text-accent-500 mt-0.5">{edu.degree} · {edu.dept}</p>
                </div>
      ...
      <section className="mb-10 pb-10 border-b border-slate-800/60">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {CERTIFICATIONS.map((cert) => (
            <div key={cert.name} className="p-4 rounded-lg border border-slate-800/60 bg-slate-900/20">
        ...
        <section className="mb-10 pb-10 border-b border-slate-800/60">
          ...
          <div className="flex flex-col gap-3">
            {PUBLICATIONS.map((pub) => (
              <div key={pub.title} className="p-4 rounded-lg border border-slate-800/60 bg-slate-900/20">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex-1 min-w-0">
      ...
      <section className="mb-10 pb-10 border-b border-slate-800/60">
        ...
        <div>
          {ACTIVITIES.map((act, i) => {
            const desc = isEn ? (act.descEn ?? act.desc) : act.desc
            return (
                  ...
                  <div>
                    {act.link ? (
                      <a href={act.link} target="_blank" rel="noopener noreferrer"
                        className="text-sm font-semibold text-slate-200 hover:text-primary-400 transition-colors">{act.title}</a>
                    ) : (
                      <p className="text-sm font-semibold text-slate-200">{act.title}</p>
                    )}
                    {act.role && <p className="font-mono text-xs text-accent-500 mt-0.5">{act.role}</p>}
                    {act.award && <p className="font-mono text-xs text-primary-400 mt-0.5">{act.award}</p>}
                  </div>
                  <span className="font-mono text-xs text-slate-600">{act.period}</span>
                </div>
                {desc && <p className="text-sm text-slate-400 leading-relaxed mt-1">{desc}</p>}
              </TimelineItem>
            )
          })}
        </div>
      ...
      <section className="mb-10 pb-10 border-b border-slate-800/60">
        ...
        <div>
          {ORGANIZATIONS.map((org, i) => {
            const desc = isEn ? (org.descEn ?? org.desc) : org.desc
            return (
              <TimelineItem key={org.name} isLast={i === ORGANIZATIONS.length - 1}>
                <div className="flex items-start justify-between flex-wrap gap-1 mb-1">
                  <div>
                    <p className="text-sm font-semibold text-slate-200">{org.name}</p>
                    {org.role && <p className="font-mono text-xs text-accent-500 mt-0.5">{org.role}</p>}
                    {org.affiliation && <p className="font-mono text-xs text-slate-600 mt-0.5">{org.affiliation}</p>}
                  </div>
            ...
            <div key={group.label} className="p-4 rounded-lg border border-slate-800/60 bg-slate-900/20">
              ...
              <div className="flex flex-wrap gap-1.5">
                {group.items.map((item) => (
                  <span key={item} className="font-mono text-xs text-slate-400 bg-slate-800/50 px-2 py-0.5 rounded">{item}</span>
                ))}
              </div>
            </div>
          ))}
